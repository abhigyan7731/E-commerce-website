...
export const uploadCart = createAsyncThunk('cart/uploadCart',
    async ({ getToken }, thunkAPI)=> {
        try {
            ...
            const response = await axios.post('/api/cart', {cart: cartItems},{ headers: {
...
const cartSlice = createSlice({
    ...
    extraReducers: (builder)=>{
        ...
        builder.addCase(fetchCart.rejected, (state, action) => {
            ...
            // Keep existing cart state on fetch failure
        })
        builder.addCase(uploadCart.rejected, (state, action) => {
            console.error('Failed to upload cart:', action.payload)
            // Keep existing cart state on upload failure
        })
    }
})
